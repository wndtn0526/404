...
| `thumbnail` `avatar-group` | Profile 1002:522932 | 원형 / 사각 4 · 그룹 40 겹침 4 |
...
⚠️ **`avatar-group` 은 `size="md"` + `overflow="count"` 가 결재선용이다.** 원본 Profile Group
(node I1002:115443) 실측 184x40 — 40 아바타 넷 + 검정 `+N` 원 하나, 흰 링 2, 겹침 4.
기본 `overflow="text"` 는 뒤에 「외 N명」 글자를 붙인다. Alpine 목록에서는 `names-expr` 을 준다.

⚠️ **결재선의 열람·참조는 갈래마다 4명까지만 줄로 늘어놓는다.** 넘으면 아바타 한 줄로 접고,
누르면 이름 목록이 뜬다 (원본 node 1002-115417 · 115425 · 115437 · 팝오버 1002-115544 210 너비).
**진행(승인)은 접지 않는다** — 순서가 결재 순서라서 뜻을 가진다(원본도 다섯이면 다섯 줄이다).
⚠️ 팝오버의 `@click.outside` 는 팝오버가 아니라 **감싸는 칸**에 건다. 팝오버에 걸면 여는 버튼이
팝오버 '밖' 이라 누르는 순간 도로 닫힌다. 갈래가 둘이라 자기 갈래가 열렸을 때만 닫게 해야
다른 갈래를 눌렀을 때 그쪽이 막 연 것을 도로 닫지 않는다.
...
- `confirm` `prompt` — Dialog 의 용도별 사전 조립. 원본은 `modal`(Dialog).
  ...
  ⚠️ 원본 확인 버튼은 순검정(#000000)인데 이 저장소 프라이머리는 Black `#111111` 이다.
     토큰을 따랐다.
- `ext-success-fill` (아이콘) — 채운 초록 원 + 흰 체크. 스낵바가 쓴다. DS `circle-check` 는
  외곽선 한 색이라 이 모양이 안 나온다. GPRO_PORTFOLIO `success_18` (node 1002:104426) 을
  그대로 옮기고 원만 `currentColor` 로 바꿨다.
- **`confirm` 과 `toast` 는 `x-layout` 이 한 번만 심는다.** 화면마다 마크업을 두지 않고
  ...
  `toast` 는 GPRO_PORTFOLIO node 1002-114358 실측으로 맞췄다 — 면 Mono/900 · 반경 6 ·
  px20 pt11 pb12 · 칸 사이 6 · 아이콘 18 · 글자 15 lh23 흰색 · GNB(56) 바로 아래 가운데.
  ⚠️ 전에는 면 자체가 초록/빨강이었다. 원본은 면이 늘 검고 아이콘만 색이 바뀐다.
  ⚠️ 원본에 닫기 버튼이 없어 뺐다. 4초 뒤 저절로 사라지고 막는 것도 없다.
  ⚠️ 실패용 아이콘은 원본에 없다. DS `circle-exclamation` 을 빨강으로 쓴다 — 성공(채운 원)과
     결이 다르다. 디자이너가 실패 스낵바를 그리면 그 글리프로 바꾼다.
  ⚠️ 폭은 원본이 335 인데 우리는 내용에 맞춘다(`w-max`). 문구가 바뀌면 335 가 맞지 않는다.
  **다음 화면에서 띄울 문구**는 `sessionStorage['cdw.toast']` 에 넣고 옮겨 간다 — 도착한 화면의
  `x-toast` 가 꺼내 띄우고 지운다. 신청 완료 → 문서 신청 목록 흐름이 이걸 쓴다(node 1002-114271).
  ⚠️ 정석은 서버 세션 플래시다. 보낼 곳이 생기면 그리로 옮기고 이 조각을 지운다.
...
`vacation`(휴가 신청 · node 1002-108468) · `vendor`(지출 결의서 거래처 · node 1002-108416).
