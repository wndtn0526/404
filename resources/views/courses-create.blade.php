{{-- 과정 추가 — 컨텐츠 관리에 올라간 컨텐츠를 골라 하나의 과정으로 묶는다.
     ...
     고른 컨텐츠에서 차시와 총 재생시간을 그 자리에서 계산해 보여준다.
     둘 다 사람이 입력하는 값이 아니라 묶은 결과다 — 손입력 칸으로 두면 표의 값과 어긋난다.

     ⚠️ 이 화면은 Figma 에 디자인이 없다. 레이아웃만 컨텐츠 추가에서 가져왔다.
     ⚠️ 차시 순서는 지금 표에 놓인 순서다. 순서를 바꾸는 UI(끌어서 정렬)는 없다.
        차시 번호가 곧 커리큘럼 순서라 실제로는 필요하다 — 디자인이 나오면 붙인다.
...
@php
    ...
    // Alpine 이 합계를 낼 때 쓰는 id → 초 대응표
    $durations = collect($contents)->mapWithKeys(fn ($c) => [$c['id'] => $c['sec'] ?? 0])->all();
    ...
    >
        <div class="mx-auto w-full min-w-0 max-w-[792px]">
            ...
            <form method="POST" action="#" class="mt-[30px] min-w-0 rounded-lg bg-background-normal p-5 lg:p-[30px]">
                ...
                {{-- ═══ 컨텐츠 구성 ═══ 이 화면의 핵심.
                     체크한 컨텐츠가 차시가 된다. 합계는 표 안(tfoot)에서 낸다 — 선택 상태
                     `selected` 가 <x-table selectable> 의 Alpine 스코프에 있어서, 표 밖에
                     두면 읽지 못한다. 안쪽 x-data 는 바깥 스코프를 그대로 본다. --}}
                <h2 class="pt-[30px] text-heading-2 font-bold leading-[30px] text-mono-black">컨텐츠 구성</h2>
                <p class="pt-2 text-label-2 leading-5 text-warm-gray-600">
                    과정에 넣을 컨텐츠를 고르세요. 고른 순서가 아니라 표에 놓인 순서가 차시 번호가 됩니다.
                </p>

                <x-table selectable min-width="660px" class="mt-[18px]">
                    <x-table.head
                        selectable
                    ...
                    {{-- 합계 — 차시는 고른 개수, 총 재생시간은 영상이 있는 것만 더한다.
                         dur 의 값은 초다. 영상이 없는 자료는 0 이라 더해도 시간이 늘지 않는다. --}}
                    <tfoot class="border-t border-line-solid-normal bg-background-alternative">
                        {{-- ⚠️ 계산은 전부 메서드로 뺀다. x-show 나 x-text 안에 화살표 함수를 직접
                             쓰면(예: selected.some(id => !dur[id])) Alpine 평가기에서 falsy 로
                             떨어진다 — 같은 식을 메서드 안에 넣고 this 로 읽으면 정상이다. --}}
                        <tr x-data="{
                                dur: @js($durations),
                                total() { return this.selected.reduce((sum, id) => sum + (this.dur[id] ?? 0), 0); },
                                silentCount() { return this.selected.reduce((n, id) => n + (this.dur[id] ? 0 : 1), 0); },
                                fmt(s) {
                                    if (! s) return '-';
                                ...
                                <div class="flex flex-wrap items-baseline gap-x-6 gap-y-1">
                                    <span class="text-label-1 text-label-alternative">
                                        ...
                                        <strong class="pl-1.5 font-bold text-label-normal tabular-nums"
                                                x-text="selected.length + '차시'">0차시</strong>
                                    </span>
                                    <span class="text-label-1 text-label-alternative">
                                        총 재생시간
                                        <strong class="pl-1.5 font-bold text-label-normal tabular-nums"
                                    ...
                                    {{-- 영상이 없는 자료를 골랐을 때만 알린다. 재생시간이 안 늘어나는
                                         이유를 화면에서 알 수 있어야 한다. --}}
                                    {{-- ⚠️ x-show 가 아니라 :class 로 감춘다. 이 자리에서 x-show 는
                                         값이 true 로 바뀌어도 다시 그려지지 않았다(표현식·스코프는
                                         정상, 같은 자리의 :class 는 반응했다). Alpine 이 show 를
                                         requestAnimationFrame 으로 미루는데 그 프레임에 기대지 않는
                                         쪽을 택했다.
                                         기본 클래스에 hidden 을 넣어 Alpine 이 붙기 전에도 감춰져
                                         있고, 못 붙어도 감춰진 채로 남는다. --}}
