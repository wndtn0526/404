...
<div x-data="{ items: @js(array_values($items)) }" class="mt-3 flex flex-wrap items-center gap-2">
    <template x-for="(item, i) in items" :key="item">
        <span class="inline-flex items-center gap-1 rounded-md border border-primary/[0.43] bg-primary/5 py-[7px] pl-[11px] pr-2 text-body-2 font-medium text-primary">
    ...
    <button type="button"
            class="inline-flex items-center gap-1 rounded-md border border-line-normal-neutral px-[11px] py-[7px] text-body-2 font-medium text-label-alternative transition-colors hover:bg-fill-alternative">
