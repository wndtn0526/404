...
@php
    ...
    /*
     * 양식 — 껍데기(머리 · 관련 파일 · 참조 문서 · 결재선 · 팝업)는 어느 양식이나 같고
     * 왼쪽 첫 카드만 다르다. 그 카드를 partials/doc-form-{양식}.blade.php 로 나눠 뒀다.
     * 양식이 늘면 조각을 하나 더 만들고 라우트에서 이름만 넘긴다.
     */
    $docForm = $docForm ?? 'expense';
    $docTitle = $docTitle ?? '지출 결의서 (개인 비용)';
...
<x-layout :title="$docTitle">
    ...
    >
        ...
        <div class="min-w-0" x-data="{
                 ...
                 /*
                  * 휴가 신청 — 종류에 따라 받는 칸이 달라진다(원본 주석 node 1002-108552).
                  *   반차   하루 · 0.5일 4시간, 시간 칸 없음
                  *   훈련·병가  날짜 + 시간 두 칸. 일수와 시간을 각 칸 아래에 따로 낸다.
                  *   그 밖    날짜(하루 또는 기간) 한 칸. 아래에 '총 N일 · N시간' 을 함께 낸다.
                  * ⚠️ 하루 8시간으로 계산한다. 원본 값(1일 8시간 · 0.5일 4시간)에서 나온 것이고
                  *    회사 규정이 다르면 여기가 아니라 서비스에서 정할 값이다.
                  */
                 leave: @js($initLeave),
                 leaveLeft() { return Math.max(0, this.leave.total - this.leave.used); },
                 halfTypes: ['오전 반차', '오후 반차'],
                 timeTypes: ['훈련 (민방위)', '병가'],
                 needsTime() { return this.timeTypes.includes(this.form.leave_type); },
                 leaveDays() {
                     if (! this.form.leave_date) return 0;
                     if (this.halfTypes.includes(this.form.leave_type)) return 0.5;
                     const parts = String(this.form.leave_date).split(' - ');
                     if (parts.length < 2) return 1;
                     const at = (s) => new Date(s.trim().replace(/\./g, '-')).getTime();
                     const span = Math.round((at(parts[1]) - at(parts[0])) / 86400000) + 1;
                     return Number.isFinite(span) && span > 0 ? span : 1;
                 },
                 dateSummary() {
                     const days = this.leaveDays();
                     if (! days) return '';
                     // 시간을 따로 받는 종류는 날짜 줄에 일수만 낸다 — 시간은 아래 칸이 말한다.
                     return this.needsTime() ? ('총 ' + days + '일') : ('총 ' + days + '일 · ' + (days * 8) + '시간');
                 },
                 timeSummary() {
                     if (! this.needsTime() || ! this.form.leave_time) return '';
                     const parts = String(this.form.leave_time).split(' - ');
                     if (parts.length < 2) return '';
                     const h = (s) => Number(s.trim().split(':')[0]) + Number(s.trim().split(':')[1] || 0) / 60;
                     const span = h(parts[1]) - h(parts[0]);
                     return span > 0 ? ('총 ' + Number(span.toFixed(1)) + '시간') : '';
                 },
            ...
            <div class="flex min-w-0 flex-wrap items-center justify-between gap-4 pt-[30px]">
                <div class="flex min-w-0 items-center gap-2">
                    ...
                    <h1 class="min-w-0 truncate text-title-2 font-bold leading-[39px] text-mono-black">{{ $docTitle }}</h1>
                </div>
            ...
            <div class="mt-[30px] flex min-w-0 flex-col gap-6 pb-10 xl:flex-row xl:items-start">
                ...
                <div class="flex min-w-0 flex-1 flex-col gap-4">
                    @include('partials.doc-form-' . $docForm)
