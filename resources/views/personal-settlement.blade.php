{{-- 개인 비용 정산 — Figma GPRO_PORTFOLIO (sJC6AduTG0I4cTttQJFAes · node 1002-92654)
     재무 > 업무 관리자 메뉴의 한 탭. 승인이 끝난 개인 비용을 정산 대기 목록으로 본다.

     원본 실측(1920) — 본문 1520
       검색·필터 카드 1520x212 · 조회 목록 카드 1520x782 (사이 16)
       조회 목록 제목 20 Bold + 「총 N건」 · 우측 버튼 셋
       표 열 80 | 80 | 100 | 350 | 350 | 140 | 120 | 120 | 180 = 1520 · 행 56 · 10행
       아래 페이지네이션 + '10개씩 보기'
     ...
     ⚠️ 입금 계좌 번호는 가운데를 가렸다. 예시 값이지만 계좌번호 모양을 그대로 두면
        어딘가에서 진짜 값이 들어와도 아무도 이상하게 보지 않는다.
...
@php
    ...
    for ($i = 1; $i <= 10; $i++) {
        $rows[] = [
            ...
            'owner' => '청담원',
            'month' => '2021.09',
            'bank' => ['국민은행', '신한은행', '하나은행'][$i % 3],
            // 가운데를 가린 예시 값
            'account' => ['3333-06-****014', '110-298-****371', '1002-955-**7206'][$i % 3],
        ];
    }
@endphp

<x-layout title="개인 비용 정산">
    <x-workspace-shell
        workspace="청담원"
    ...
    >
        ...
        <x-filter-bar
            search="업무 번호 · 문서 이름 검색"
            :active="['month', 'state']"
            :columns="[
                ['key' => 'month', 'label' => '정산월', 'type' => 'date'],
                ['key' => 'dept', 'label' => '귀속 부서', 'type' => 'select', 'options' => ['개발팀', '운영팀', '콘텐츠팀']],
                ...
                <x-table selectable min-width="1520px" class="rounded-none border-x-0">
                    <x-table.head
                        ...
                        :columns="[
                            ...
                            ['label' => '정산월', 'width' => '120px'],
                            ['label' => '은행 이름', 'width' => '120px'],
                            ['label' => '입금 계좌 번호'],
                        ]"
                    />
                    <tbody>
                        @forelse ($rows as $row)
                            <x-table.row selectable :value="$row['no']">
                                ...
                                <x-table.cell tone="muted" nowrap>
                                    <span class="tabular-nums">{{ $row['month'] }}</span>
                                </x-table.cell>
                                <x-table.cell tone="muted" nowrap>{{ $row['bank'] }}</x-table.cell>
                                <x-table.cell tone="muted" nowrap>
                                    <span class="tabular-nums">{{ $row['account'] }}</span>
                                </x-table.cell>
                            </x-table.row>
                        @empty
                            <x-table.empty :colspan="9">정산할 개인 비용이 없습니다.</x-table.empty>
                        @endforelse
                    </tbody>
                </x-table>
            </div>
